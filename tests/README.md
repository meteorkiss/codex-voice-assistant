...
更新：2026-09-21。以下命令从项目根目录执行，使用 Windows PowerShell 5.1；WPF 测试带 `-STA`。测试样本在 `tests/fixtures`，运行产物写入 `work/tests` 或 `work/desktop-shell-render`。不应使用个人 `data` 作为测试输出目录。

独立审核追加：`Test-HandsFree.ps1` 837 断言通过。原测试只覆盖先回执再回答；新增逆序 wake/follow-up 两种来源，回答文字先显示但不在 accepted 前朗读，正常/立即自然播放完成后均能接话，并注入 Tick 中途 worker 退出；拒绝/未知及目标/绑定/input/voice/token/新草稿/取消/关闭/后来轮次不清新稿、不重派或重开。worker 已退出但尚未分类的用例走实际 Tick 读取缺失回执，仍保留未知锁。生产账本状态转换、旧两轮链路继续运行；新测试的播放次数/epoch 由替身按真实完成约定计数，无真实音频或消息。相关归档恢复 88、语音切换 380、自动连接 215、源码声明 46 同轮通过；新增产物自清理，原受阻边界不变。

2026-09-21 接话回归：`Test-HandsFree.ps1` 482 断言通过。新增场景让桥接替身真正调用生产 Set/Sync/Clear-PendingSend（只替代持久化），覆盖两轮 ASR→在途锁→回执清稿→回答排队/模拟播放→再次接话。原替身未产生 pending 状态，未覆盖该现场故障。九种失效上下文及拒绝/未知回执继续保护新草稿、不重派。相关归档恢复 88、语音切换 380、自动连接 215、源码声明 46 通过；设备/语音/桥接均为合成替身，无真实任务或录音测试。新产物自清理，原 27 个受阻文件及旧组合边界不变。

本地近似候选确认增量：口令 1543、匹配 50、桥接 53、独立 `Test-SpokenTaskSwitch.ps1` 13 场景/380 断言、免手 340、归档恢复 88、手动自动连接 166、源码声明 46 通过。专项覆盖真实生产语音识别结果到确认/读取/绑定路径，英文拼写偏差和任务里尾缀、是/否、多候选不代选、过期/换目标/新普通话题撤销，未调用真实任务或录音。相似度仅作候选召回，各关键词和数字硬约束保持；既有别名/同音改为需确认。复用标准库及现有 pypinyin，无新增依赖；旧组合入口仍未运行，未重试历史受阻清理。

2026-09-13 截图语音切换增量：口令 1515、匹配 43、桥接 53、`Test-SpokenTaskSwitch.ps1` 8 场景/189 断言、免手 340、归档恢复 88、源码声明 46 项独立通过。新专项复用真实生产醒词/提问生命周期及 Tick→ASR→本地路由→匹配→读取/绑定，真实 Python 只调用纯匹配器；设备和桥接均替身。覆盖截图的“申办0.6.17”“6.117”、下一句只投递新编号、重名不猜、新输入使旧回执失效、查找/读取失败不转聊天。新增自有随机目录内的生成文件已精确清理，未触碰原 27 个受阻文件。

新专项需显式提供 Python，不加入仍未运行的旧复制/组合入口；纯源码检出可指向自行准备的解释器，不能为运行测试复制个人 runtime 或 data。例如已准备项目运行时的安装目录可执行：

```powershell
powershell.exe -NoProfile -NonInteractive -STA -ExecutionPolicy Bypass -File tests/Test-SpokenTaskSwitch.ps1 -PythonPath .\runtime\python\python.exe
```
